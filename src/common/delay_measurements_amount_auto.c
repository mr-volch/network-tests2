...
{
  double *rfft_res = (double*)malloc(window_length*sizeof(double));
  double *window_power_spectrum = (double*)malloc(power_spectrum_length*sizeof(double));
  rfftw_plan p;
  int k;

  p = rfftw_create_plan(window_length, FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE);
  rfftw_one(p, window, rfft_res);
  window_power_spectrum[0] = rfft_res[0]*rfft_res[0];  /* DC component */
  for (k = 1; k < (window_length+1)/2; ++k)
  { /* (k < N/2 rounded up) */
    window_power_spectrum[k] = rfft_res[k]*rfft_res[k] + rfft_res[window_length-k]*rfft_res[window_length-k];
  }
  if (window_length % 2 == 0)
  { /* N is even */
    window_power_spectrum[window_length/2] = rfft_res[window_length/2]*rfft_res[window_length/2];  /* Nyquist freq. */
  }
